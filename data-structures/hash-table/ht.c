...
static const char *ht_set_entry(ht_entry *entries, size_t capacity, const char *key, void *value, size_t *plength)
{
    ...
    while (entries[index].key != NULL)
    {
        ...
        {
            // Found key (it already exists), update it
    ...
    if (plength != NULL)
    {
        // If the plength arg is non-NULL, it's being called from ht_set,
...
{
    // AND has with capacity-1 to ensure it's within entries array
    uint64_t hash = hash_key(key);
    ...
    while (table->entries[index].key != NULL)
    {
        ...
        // Key wasn't in this slow, move to next (linear probing)
        index++;
